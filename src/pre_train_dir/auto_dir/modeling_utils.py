...
class PreTrainedModel(nn.Module, ModuleUtilsMixin):
    r""" Base class for all scAuto.

        :class:`~transformers.PreTrainedModel` takes care of storing the configuration of the scAuto and handles methods for loading/downloading/saving scAuto
        as well as a few methods common to all scAuto to (i) resize the input embeddings and (ii) prune heads in the self-attention heads.
    ...
    def get_output_embeddings(self):
        ...
        """
        return None  # Overwrite for scAuto with output embeddings
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        ...
        if from_tf:
            ...
            # Make sure we are able to load base scAuto as well as derived scAuto (with heads)
            start_prefix = ""
            model_to_load = model
    ...
    def generate(
        ...
        r""" Generates sequences for scAuto with a LM head. The method currently supports greedy or penalized greedy decoding, sampling with top-k or nucleus sampling
        and beam-search.
